...
class ConfidenceScore(ValueObject):
    """Score 0–100 with derived label and human-readable justification.

    Use ConfidenceScore.from_value() as the preferred constructor — it
    derives the label automatically from the numeric score.
    """
...
class ConfidenceFactors(ValueObject):
    """Weighted input factors for the confidence scoring engine.

    Each factor is a normalized value [0.0, 1.0]. The ConfidenceEngine
    (domain service) applies weights and produces a ConfidenceScore.
    """
